    ...
    const handleScanLegacy = useCallback(
        (result: ScanResult) => {
            ...
            if (areFramesComplete(newFrames)) {
                // Recover the original token string. Binary payloads (v1 base64
                // bytes) are re-encoded as base64; text payloads (v2 `fedimint…`
                // utf8 bytes) are decoded as utf8.
                const strData = decodeEcashFromBuffer(framesToData(newFrames))
